namespace Marvel.Tests
{
    ...
    public class MarvelStoriesTests
    {
        ...
        [Test]
        public async Task Api_Should_Get_Series_For_First_Story()
        {
            var stories = await _marvel.GetStories(new CancellationToken());
            Assert.NotNull(stories);
            var story = stories.Container.Results.FirstOrDefault();
            ...
            var json = await _marvel.GetStorySeriesJson(story.Id, new CancellationToken());
            Assert.NotNull(json);

            var model = await _marvel.GetStorySeries(story.Id, new CancellationToken());
            Assert.NotNull(model);
        }
    }
}